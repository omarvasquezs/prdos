...
use App\Http\Controllers\Api\PadronRucController;
...
Route::middleware(['web', 'auth'])->withoutMiddleware([\Illuminate\Foundation\Http\Middleware\VerifyCsrfToken::class])->group(function () {
    ...
    Route::post('/api/comprobantes/{codComprobante}/anular', [ComprobanteController::class, 'anular']);
    ...
    Route::post('/api/pedidos-cola/{pedido}/costo-delivery', [PedidoController::class, 'updateDeliveryCost']);

    // Padrón RUC (SUNAT)
    Route::get('/api/padron/buscar', [PadronRucController::class, 'buscar']);
    Route::get('/api/padron/ruc/{ruc}', [PadronRucController::class, 'buscarRuc']);
    Route::get('/api/padron/stats', [PadronRucController::class, 'stats']);
});
