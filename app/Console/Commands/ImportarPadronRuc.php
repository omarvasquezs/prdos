    ...
    public function handle(): int
    {
        ...
        try {
            ...
            while (($line = fgets($handle)) !== false) {
                ...
                $parts = explode('|', $line);
                // Formato: RUC|NOMBRE|ESTADO|CONDICION|...(varios campos extras ignorados)
                $ruc = trim($parts[0] ?? '');
                $nombre = trim($parts[1] ?? '');
                $estado = trim($parts[2] ?? '');
                $condicion = trim($parts[3] ?? '');

                if (strlen($ruc) < 8 || strlen($ruc) > 11)
    ...
    private function downloadFile(string $url, string $dest): void
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_PROGRESSFUNCTION => function ($resource, $downloadSize, $downloaded) {
                if ($downloadSize > 0) {
                    // Progreso silencioso; se puede mejorar si se desea
                }
            },
        ]);
        $result = curl_exec($ch);
